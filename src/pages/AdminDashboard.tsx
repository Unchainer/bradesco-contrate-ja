...
import { LogOut, Download, RefreshCw, Search, ShieldCheck } from "lucide-react";
  ...
  const filtered = search
    ? leads.filter(
        (l) =>
          ...
          l.insurance_type.toLowerCase().includes(search.toLowerCase())
      )
    : leads;
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-12 text-muted-foreground">
                      {search ? "Nenhum lead encontrado." : "Ainda não há leads cadastrados."}
                    </TableCell>
                  </TableRow>
                ) : (
                  filtered.map((lead) => (
